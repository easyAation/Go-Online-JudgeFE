      ...
      <Col :span="23">
        <draggable v-model="contest.list">
          <transition-group name="list">
            <div v-for="(item, index) in contest.list" :key="item.id" class="list-item">
              <div>{{ index+1 }} -- {{item.id}} {{ item.name }}</div>
              <!-- <Icon type="close-circled" @click.native="removeJob(index)"></Icon> -->
              <Button type="primary" @click="removeJob(index)">删除</Button>
            </div>
          </transition-group>
        </draggable>
      </Col>
  ...
  created() {
    ...
    if (this.$route.params.cid) {
      ...
      if (this.overview.length === 0) {
        p = this.$store.dispatch("contest/findOne", this.$route.params, "cid");
      }
      p.then(() => {
  ...
  methods: {
    add() {
      http
        ...
        .then(res => {
          this.contest.list.push(res.data.problem);
          this.pid = "";
        })
        .catch(err => {
      ...
      // this.$store
      //   .dispatch("problem/findOne", only(this, "pid"))
      //   .then(({ problem }) => {
